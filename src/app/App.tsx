  ...
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" fill="none" style={{ display: 'block' }}>
      <defs>
        ...
        {/* Inner petal — warm gold */}
        <radialGradient id={`${uid}p3`} cx="50%" cy="55%" r="55%">
          <stop offset="0%"   stopColor="#F0C84A" stopOpacity="1"/>
          <stop offset="40%"  stopColor="#E0A840" stopOpacity="0.95"/>
          <stop offset="100%" stopColor="#C4722A" stopOpacity="0.8"/>
        </radialGradient>
      ...
      <path d="M100 74 C100 74 80 106 80 122 C80 138 89 150 100 150 C111 150 120 138 120 122 C120 106 100 74 100 74Z"
            fill={`url(#${uid}p3)`}/>
      {/* NO top dot or white highlight on hero/waitlist logo */}
    </svg>
  );
}
    ...
    <div style={{ background: C.parchment, fontFamily: CG, minHeight: '100vh' }}>
      ...
      <section style={{ maxWidth: '680px', margin: '0 auto', padding: '0 32px 60px', textAlign: 'center' }}>
        <div style={{ marginBottom: '44px' }}>
          {([
            ['Ancient wisdom always knew — your body was never broken.', false],
            ['It was always communicating.', true],
          ] as [string, boolean][]).map(([line, accent], i) => (
            <div key={i} style={{
              fontFamily: CG, fontSize: '28px', fontWeight: 300,
              lineHeight: 1.75, letterSpacing: '0.3px',
              color: accent ? C.terracottaDeep : C.ink,
        ...
        <p style={{ fontFamily: CG, fontSize: '16px', fontWeight: 300, lineHeight: 2, color: C.wt1, marginBottom: '6px' }}>
          In Sanskrit, <em style={{ color: C.terracotta }}>Nadi</em> means channel. The path through which your life force moves.
        </p>
        <p style={{ fontFamily: CG, fontSize: '16px', fontWeight: 300, lineHeight: 2, color: C.wt1, marginBottom: '36px' }}>
          Your energy doesn't hide — it flows. It just needs to be heard.
        </p>
